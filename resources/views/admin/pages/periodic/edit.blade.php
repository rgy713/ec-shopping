...
@section('title') {{__("admin.page_title.periodic_edit")}} @endsection

@section('contents')
    <form method="POST" id="edit_basic_info_form" accept-charset="UTF-8" action="{{route("admin.periodic.edit_info")}}">
        {{ csrf_field() }}
        ...
        <div class="row" id="order_info_body">
            ...
            <div class="col-6" id="shop_memo_info_order">
                @include('admin.components.common.shop_memo_input_order',['shopMemos'=>isset($orderShopMemos) ? $orderShopMemos : null, 'prefix'=>'order', 'shopMemoTitle'=>__("admin.item_name.periodic.order_memo"),'withShopMemoInput'=>true,'withShopMemoList'=>true,'shopMemoHeight'=>'186px'])
            </div>
            ...
            <div class="col-12">
                <div class="row mb-3">
                    <div class="col-6">
                        <a class="btn btn-sm btn-block btn-secondary" tabindex="-1" href="@if(isset($order) && isset($order['old_customer_id'])){{route('admin.customer.info', ['back'=>true])}}@else{{route('admin.periodic.search', ['back'=>true])}}@endif">
