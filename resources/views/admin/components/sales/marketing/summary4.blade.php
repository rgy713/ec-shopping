            ...
            <div class="col-12">
                ...
                <table class="table table-sm table-bordered">
                    ...
                    @foreach($summary as $key=>$item)
                        @if ($key == 0)
                            <tr>
                                <th>{{__("admin.item_name.common.sum")}}</th>
                                <td>{{$total}}{{__("admin.item_name.common.man_unit1")}}</td>
                                <td>100%</td>
                            </tr>
                        @elseif ($item->counts > 0)
                            <tr>
                                <th>@if ($item->counts == 1){{__("admin.item_name.sales.first_time")}}@else {{$item->counts}}{{__("admin.item_name.sales.count_unit")}}@if ($item->counts == 10){{__("admin.item_name.common.greater")}}@endif @endif</th>
