...
@section('contents')
    <form method="POST" id="edit_basic_info_form" accept-charset="UTF-8"
          action="@if(isset($order) && isset($order['order_id'])){{route('admin.periodic.edit_save')}}@else{{route('admin.periodic.create_save')}}@endif">
        {{ csrf_field() }}
        ...
        <div class="row" id="order_info_body">
            <div class="col-12">
                @include('admin.components.periodic.edit_basic_info')
            </div>
            ...
            <div class="col-12" id="edit_order_delivery_info_form">
                @include('admin.components.order.edit_order_delivery_info')
            </div>
            {{-- 配送/支払い方法 end --}}
                ...
                <div class="row mb-3">
                    <div class="col-6">
                        <a class="btn btn-sm btn-block btn-secondary" tabindex="-1"
                           href="@if(isset($order) && isset($order['order_id'])){{route('admin.periodic.edit', ['id'=>$order['order_id'], 'back'=>true])}}@else{{route('admin.periodic.create', ['back'=>true])}}@endif">
